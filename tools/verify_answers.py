...
def get_question_id(filename: str) -> int:
    if len(filename) <= 4:
        raise ValueError
    ...
    last_i = 0
    for i, char in enumerate(reversed(filename), start=1):
        if not char.isdigit():
            break
        last_i = i
    return int(filename[-last_i:])
...
def verify_submission():
    ...
    for f in files:
        ...
        try:
            ...
            question_id = get_question_id(filename)
        except (requests.RequestException, ValueError, KeyError, IndexError) as e:
            print(
                f"Error: Could not read or parse the submission file: {filename}({e})"
            )
            sys.exit(1)
        ...
        correct_answer = correct_answers.get(question_id) or correct_answers.get(
            str(question_id)
        )
